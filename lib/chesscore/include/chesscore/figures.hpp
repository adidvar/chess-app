...
class Position {
 public:
  ...
  constexpr Position() noexcept : m_index(k_invalid_position) {}

  constexpr Position(uint8_t index) noexcept : m_index(index) {}
  ...
  [[nodiscard]] constexpr bool isValid() const noexcept {
    return m_index < k_invalid_position;
  }

  [[nodiscard]] constexpr uint8_t index() const noexcept {
    return m_index;
  }
...
class Figure {
 public:
  ...
  constexpr Figure(int8_t value) noexcept : m_value(value) {}
  ...
  constexpr bool isValid() const noexcept {
    return m_value >= BKing && m_value <= WKing;
  }
  ...
  constexpr int8_t toIndex() const noexcept {
...
class Color {
 public:
  constexpr Color(int8_t value) noexcept : m_value(value) {}
...
class Turn {
 public:
  ...
  constexpr Turn(Position from, Position to, bool advanced = true);
  constexpr Turn(Position from, Position to, Figure figure,
                 bool advanced = true);
 ...
 private:
  uint16_t m_from : 6;
  uint16_t m_to : 6;
  uint16_t m_advanced : 1;
  uint16_t m_figure : 3 = Figure::Empty;
};

static_assert(sizeof(Turn) == 2, "Turn must be exactly 2 bytes!");

constexpr Turn::Turn() : m_from(0), m_to(0), m_advanced(true), m_figure(7) {}

constexpr Turn::Turn(Position from, Position to, bool advanced)
    : m_from(from.index()), m_to(to.index()), m_advanced(advanced) {}

constexpr Turn::Turn(Position from, Position to, Figure figure, bool advanced)
    : m_from(from.index()),
      m_to(to.index()),
      m_advanced(advanced),
      m_figure(figure) {}
...
constexpr bool Turn::isAdvanced() const noexcept {
  return m_advanced;
}
