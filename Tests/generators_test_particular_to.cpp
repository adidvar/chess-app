...
static size_t MovesCounter(Board board,size_t depth, uint64_t maskto = 1){
    if(!depth)
        return 1;
    size_t counter = 0;
    ...
    for(auto board : boards1){
        counter += MovesCounter(board,depth-1,maskto);
    }
...
TEMPLATE_TEST_CASE( "Particular subboards generation test to", "[particular][bitboard][board][fen]", BitBoard){
...
TEMPLATE_TEST_CASE( "Particular subboards generation advanced test to", "[particular][bitboard][board][fen]", BitBoard){
