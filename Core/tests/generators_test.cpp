...
TEMPLATE_TEST_CASE( "Boards moves generation test", "[bitboard][board][fen]", BitBoard){
   ...
   REQUIRE(TestType("8/8/2p5/8/2R1P3/8/8/8 w - - 0 1").GenerateTurns().size() == 9);
   REQUIRE(TestType("2q5/8/8/8/8/8/4P3/3K4 w - - 0 1").GenerateTurns().size() == 4);
   REQUIRE(TestType("8/8/8/4pP2/8/8/8/8 w - e6 0 1").GenerateTurns().size() == 2);
...
TEMPLATE_TEST_CASE( "Boards moves generation advanced test", "[bitboard][board][fen]", BitBoard){
