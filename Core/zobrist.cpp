...
uint64_t GetTurnCounterHash(int count)
{
    return 0;
}

uint64_t GetPassiveCountHash(int count)
{
    return 0;
}

uint64_t GetCurrentColorHash(Color color)
{
    return CurrentColorHash[color];
}
...
void InitZobrist()
{
    ...
    for(size_t c = 0; c < Color::Max(); ++c)
        ShortCastlingFlagHash[c] = random();
    for(size_t c = 0; c < Color::Max(); ++c)
        LongCastlingFlagHash[c] = random();
    for(size_t c = 0; c < Color::Max(); ++c)
        CurrentColorHash[c] = random();
    for(size_t p = 0; p < Position::Max(); ++p)
        ElpassantHash[p] = random();
}
